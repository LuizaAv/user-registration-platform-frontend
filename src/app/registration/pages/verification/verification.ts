...
import { PreferredCompanySize, EducationLevel } from '../../../shared/enums/enum';
...
export class Verification implements OnInit {
  ...
  private loadReferenceData() {
    this.api.getIndustries().subscribe({
      next: (response) => {
        this.industries = response.data.reduce((acc, item) => {
    ...
    this.api.getTimeZones().subscribe({
      next: (response) => {
        this.timezones = response.data.reduce((acc, item) => {
          acc[item.id] = item.name;
          return acc;
        }, {} as { [id: string]: string });
      }
    ...
    this.api.getSkills().subscribe({
      next: (response) => {
        this.skills = response.data.reduce((acc, item) => {
          acc[item.id] = item.name;
          return acc;
        }, {} as { [id: string]: string });
      }
    ...
    this.api.getCareerGoals().subscribe({
      next: (response) => {
        this.careerGoals = response.data.reduce((acc, item) => {
          acc[item.id] = item.name;
          return acc;
        }, {} as { [id: string]: string });
      }
    ...
    this.api.getJobTitles().subscribe({
      next: (response) => {
        this.jobTitles = response.data.reduce((acc, item) => {
          acc[item.id] = item.name;
          return acc;
        }, {} as { [id: string]: string });
      }
  ...
  private getJobTitleName(jobTitleId: string): string {
    return this.jobTitles[jobTitleId] || jobTitleId;
  }
  ...
  submitRegistration() {
    ...
    combineLatest([
    ...
    ]).pipe(take(1)).subscribe(([personalInfo, professionalInfo, preferences]) => {
      const registrationData = {
        ...
        gender: personalInfo.gender ? personalInfo.gender.charAt(0).toUpperCase() + personalInfo.gender.slice(1) : personalInfo.gender,
        ...
        skills: professionalInfo.skills ? professionalInfo.skills.map((skillId: string) => this.skills[skillId] || skillId) : [],
        linked_in_URL: professionalInfo.linkedinUrl,
        career_goals: preferences.careerGoals ? preferences.careerGoals.map((goalId: string) => this.getCareerGoalName(goalId)) : [],
        preferred_company_size: preferences.companySize ? preferences.companySize.map((sizeId: string) => this.mapCompanySize(sizeId)) : [],
        availability_date: preferences.availabilityDate,
        time_zone: this.timezones[preferences.timezone] || preferences.timezone,
        languages: [{ language: 'English', level: 'Fluent' }], 
        newsletter_subscription: ['All Off'], 
      ...
      this.api.submitRegistration(registrationData).subscribe({
        next: (response) => {
          ...
          if (response.statusCode === 201) {
            this.router.navigate(['/registration/complete', response.newRegistration.id], { state: { registrationData: response.newRegistration } });
          } else {
